<script context="module">
  const DRM_TOKEN_TTL_MS = 5 * 60 * 1000;
  ...
  onMount(async () => {
    try {
      ...
      shakaPlayer.addEventListener('error', (event) => {
        ...
        const currentCh = get(playingChannel);
        if (currentCh?.encryption_url) {
          invalidateCachedDrmToken(currentCh.encryption_url, currentCh.id);
        }
